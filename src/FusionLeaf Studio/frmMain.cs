...
namespace FLS
{
    public partial class frmMain : Form
    {
        ...
        private bool testPort(int port)
        {
            ...
            {
                MessageBox.Show("Port " + port + " is not available.");
                btnStart.Text = "Stop";
                return false;
            }
            return true;
        ...
        {
        	simpleOpen(Path.GetTempPath());
        }

        private void webRootwwwToolStripMenuItem_Click(object sender, EventArgs e)
        {
        	simpleOpen(Path.Combine(strHomeDir,@"webroot"));
        }

        private void authorJoeSpurrierToolStripMenuItem_Click(object sender, EventArgs e)
        {
        ...
        private void NiTaskbarMouseDoubleClick(object sender, MouseEventArgs e)
        {
        	maximizeForm();
        }
        ...
        {
        	string output = "";
        	for(int i = (int)numFCGIPort.Value;i<numFCGIPort.Value+count;i++)
        	{
        ...
        {        	
        	...
        	{
	        	blNginxRunning = processIsRunning("nginx");
	        	if (blNginxRunning)
	        	{
        	...
        	//txtNginxStatus.Invoke((MethodInvoker)delegate(){this.txtNginxStatus.Text = "Done";});
        }
        
        private void BtnNginxStartClick(object sender, EventArgs e)
        {
        	if (!isReady()) return;
        	if (!processIsRunning("nginx"))
        	{
    			...
    			alReplace.Add(new string[]{"%WEBROOT%", strWebRoot.Replace(@"\","/")});
    			alReplace.Add(new string[]{"%WEBPORT%", numNginxPort.Value.ToString()});
    			alReplace.Add(new string[]{"%THREADS%", buildFarm((int)numFCGIThreads.Value)});
    			
    			//File.WriteAllText(strNginxOut, File.ReadAllText(strTemplate).Replace("%WEBROOT%",strWebRoot.Replace(@"\","/")));
    			File.WriteAllText(strNginxOut, replaceText(File.ReadAllText(strTemplate),alReplace));
        		
        		runCmd(fileNginx,"", folderNginx);
        		txtStatus.Text = "Nginx started on port " + numNginxPort.Value.ToString() + ".";
        		if (!blNginxThreadRun)
        		{
        			blNginxThreadRun = true;
        			tNginx = new Thread(new ThreadStart(monitorNginx));
        			tNginx.IsBackground=true;
        			tNginx.Start();
        			
        			//txtStatus.Text = "Nginx monitoring thread started.";
        		}
        		//else txtStatus.Text = "Nginx monitoring thread already started.";
        	}
        	else txtStatus.Text = "Nginx is already running.";
        }
        ...
        private void BtnNginxReloadClick(object sender, EventArgs e)
        {
        	if (processIsRunning("nginx"))
        	{
        		runCmd(fileNginx,"-s reload", folderNginx);
        		txtStatus.Text = "Nginx reloaded.";
        	}
        	else txtStatus.Text = "Nginx is not currently running.";
        }
        
        private void BtnNginxReopenClick(object sender, EventArgs e)
        {
    		if (processIsRunning("nginx"))
        	{
        		runCmd(fileNginx,"-s reopen", folderNginx);
        		txtStatus.Text = "Nginx reopened the logs.";
        	}
        	else txtStatus.Text = "Nginx is not currently running.";
        }
        ...
        {        	
        	int intStartCount = (int)numFCGIThreads.Value;
        	Hashtable htLocal = new Hashtable();
        	
			foreach(Object o in htFCGI.Keys)
			{
				htLocal.Add(o,htFCGI[o]);
			}
        	
        	while(blFCGIRunning || !blFCGIKill)
        	{
	        	blFCGIRunning = processIsRunning("php-cgi");
	        	if (blFCGIRunning) txtFCGIStatus.Invoke((MethodInvoker)delegate(){this.txtFCGIStatus.Text = "Running "+processIsRunningCount("php-cgi")+" thread(s)";});
	        	else txtFCGIStatus.Invoke((MethodInvoker)delegate(){this.txtFCGIStatus.Text = "Stopped";});
	        	
	        	if (intStartCount!=processIsRunningCount("php-cgi") && blFCGIKill==false)
	        	{
	        		Process[] pLIst = System.Diagnostics.Process.GetProcessesByName("php-cgi");
	        		
	        		Hashtable htTemp = (Hashtable)htLocal.Clone();
	        		
	        		foreach(Object o in htTemp.Keys)
	        		{
	        			bool blFound = false;
		        		...
		        		if (!blFound)
		        		{
		        			int intPID = runCmd(fileFCGI,"-b 127.0.0.1:" + htTemp[o].ToString(), folderFCGI);
		        			htLocal.Remove(o);
		        			htLocal.Add(intPID,(int)htTemp[o]);
		        			txtStatus.Text = "FastCGI thread restarting on 127.0.0.1:"+htTemp[o].ToString()+" with PID "+intPID.ToString()+".";
		        		}
	        		}
	        	}
	        	Thread.Sleep(threadSleep);
        	...
        	blFCGIThreadRun = false;
        }
        
        private void BtnFCGIStartClick(object sender, EventArgs e)
        {
        	if (!isReady()) return;
        	if (!processIsRunning("php-cgi"))
        	{
        		...
        		{
        			int intPID = runCmd(fileFCGI,"-b 127.0.0.1:" + j, folderFCGI);
        			htFCGI.Add(intPID,j);
        			txtStatus.Text = "FastCGI thread starting on 127.0.0.1:"+j+" with PID "+intPID.ToString()+".";
        		}
        		
        		txtStatus.Text = "FastCGI started "+numFCGIThreads.Value+" thread(s).";
        		if (!blFCGIThreadRun)
        		{
        			blFCGIThreadRun = true;
        			tFCGI = new Thread(new ThreadStart(monitorFCGI));
        			tFCGI.IsBackground=true;
        			tFCGI.Start();
        			
        			//txtStatus.Text = "FastCGI monitoring thread started.";
        		}
        		//else txtStatus.Text = "FastCGI monitoring thread already started.";
        	}
        	else txtStatus.Text = "FastCGI is already running.";
        }
        
        private void BtnFCGIStopClick(object sender, EventArgs e)
        {
        	if (processIsRunning("php-cgi"))
        	{
        		blFCGIKill=true;
        		runCmd("taskkill","/im php-cgi.exe /f", "");
        		txtStatus.Text = "FastCGI stopped.";
        		//if (!blFCGIThreadRun) txtStatus.Text = "FastCGI monitoring thread already stopped.";
        	}
        	else txtStatus.Text = "FastCGI is not currently running.";
        ...
        {
        	bool blLoop = true;
        	while(blMySQLRunning || blLoop)
        	{
	        	blMySQLRunning = processIsRunning("mysqld");
	        	if (blMySQLRunning)
	        	{
        ...
        private void BtnMySQLStartClick(object sender, EventArgs e)
        {
        	if (!isReady()) return;
        	if (!processIsRunning("mysqld"))
        	{
        		...
        		if (!blMySQLThreadRun)
        		{
        			...
        			tMySQL = new Thread(new ThreadStart(monitorMySQL));
        			tMySQL.IsBackground=true;
        			tMySQL.Start();
        			
        			//txtStatus.Text = "MySQL monitoring thread started.";
        		}
        ...
        private void RestoreToolStripMenuItemClick(object sender, EventArgs e)
        {
        	if (!processIsRunning("mysqld"))
        	{
        		MessageBox.Show("MySQL must be running to restore","MySQL Problem");
        		return;
        	}
            string strHidden = Path.Combine(strHomeDir,@"webroot\localhost\_hidden");
            string strFile = Path.Combine(strHidden,"site");
            
            if (File.Exists(strFile))
            {            	
            	string strMySQLBackup = Path.Combine(strHidden,"mysql_backup");
            	if (!Directory.Exists(strMySQLBackup))
            	{
            		MessageBox.Show("No backup folder found","Restore Problem");
            		return;
            	}
            	string[] strFiles = Directory.GetFiles(strMySQLBackup);
            	
            	if (strFiles.Length>0)
            	{
	            	string strNewestRestore = strFiles[strFiles.Length-1];
	            	FileInfo fi = new FileInfo(strNewestRestore);
	            	string strDBUser = grabValue(strFile,"db_root_username");
	            	string strDBPass = grabValue(strFile,"db_root_password");
	            	string strDBDatabase = grabValue(strFile,"db_database");
	            	
	            	string strCmd = Path.Combine(folderMySQL,"mysql.exe");
	            	string strArgs = "--user "+strDBUser+" --password="+strDBPass+" --port="
	            		+numMySQLPort.Value+" "+strDBDatabase+" < \""+strNewestRestore+"\"";
	            	DialogResult dr = MessageBox.Show("Are you sure you want to restore the latest backup?\r\n\r\n"+fi.Name,"Confirm Restore",
	            	                                  MessageBoxButtons.YesNo,MessageBoxIcon.Question);
	            	if (dr== DialogResult.Yes)
	            	{
	            		string strTempFile = "temp.cmd";
	            		File.WriteAllText(strTempFile,"@ECHO OFF\r\n\""+strCmd+"\" "+strArgs+"\r\ndel temp.cmd");
	            		runCmd(strTempFile,"","");
	            		MessageBox.Show("Restore finished. Be sure to run a /recache","Complete");
	            		if (File.Exists(strTempFile)) File.Delete(strTempFile);
	            		
	            	}else MessageBox.Show("Restore canceled","No Change");
            	}
            	else MessageBox.Show("No backup files found","Restore Problem");
